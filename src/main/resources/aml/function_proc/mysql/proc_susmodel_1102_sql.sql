  ...
  insert into aml_transaction_run
    select *
     ...
     where suspcs_flag = '1'
       ...
       where concat(atr.trans_nationality,atr.trans_address) in (select concat(t.nationality_code,t.address_area) from Aml_High_Risk_Area t) and 1 = (select enable from aml_stcr where featurecode = '80101');
    ...
    -- 对公客户注册地
      ...
      select atr.trans_code trans_code, -- 交易流水编号
       ...
       where concat(acpg.nationality_code1,acpg.register_area) in (select concat(t.nationality_code,t.address_area) from Aml_High_Risk_Area t)
    ...
    -- 对公客户经营地
      ...
      select atr.trans_code trans_code, -- 交易流水编号
       ...
       where concat(acpg.management_nationality_code1,acpg.management_area1) in (select concat(t.nationality_code,t.address_area) from Aml_High_Risk_Area t)
    ...
    -- 对私客户出生地
       ...
       where concat(acps.nationality_code1,acps.register_area) in (select concat(t.nationality_code,t.address_area) from Aml_High_Risk_Area t)
    ...
    -- 对私客户居住地
      insert into aml_black_trans
       ...
       where concat(acps.management_nationality_code1,acps.management_area1) in (select concat(t.nationality_code,t.address_area) from Aml_High_Risk_Area t)
    ...
    -- 交易对手所属地
      insert into aml_black_trans
       ...
       where concat(atr.other_customer_nationality,atr.other_customer_area) in (select concat(t.nationality_code,t.address_area) from Aml_High_Risk_Area t)
