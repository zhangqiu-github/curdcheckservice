CREATE DEFINER=`aml`@`%` PROCEDURE `proc_susmodel_1201`(in dt_date varchar(20))
BEGIN
    /*
  可疑模型代码：1201
  ...
  */ 
	...
	select paramValue into black_recall_day from aml_modelparam where paramCode = 'black_recall_day';
	set black_recall_day = cast(black_recall_day as decimal(10,0));
		drop table if exists aml_black_trans;
	  create table aml_black_trans
    (
  ...
  select cast(dt_date as date) into v_dt_date;
   ...
   insert into aml_transaction_run
    select * from aml_transaction
     where suspcs_flag = '1'
       and date_id between DATE_ADD(v_dt_date, INTERVAL -black_recall_day YEAR) and v_dt_date;
  ...
  select case when count(1) = 0 then 
   ...
   where date_id = v_dt_date and suspcs_model_code = '1201';
        ...
        concat('交易金额',trans_amount_cny,'元') v_stcb, -- 资金交易及客户行为情况
       ...
       where (concat(ab1.id_type , ab1.id_num) is not null or
       ...
       and (select featureCodes from aml_susmodel where susModelCode = '1201') like '%80021%';
        ...
        inner join (select * from Aml_Blacklist where id_num is not null or id_num <> '') ab3
          on concat(acpg.legal_crtft_type ,  acpg.legal_crtft_number)  =
             concat(ab3.id_type ,  ab3.id_num) 
         and ab3.status = '1' and ab3.back_flag = '0'
        where 1 = (select enable from aml_stcr where featurecode = '80022')
       and (select featureCodes from aml_susmodel where susModelCode = '1201') like '%80022%';
        ...
        inner join (select * from Aml_Blacklist where id_num is not null or id_num <> '') ab4
          on concat(acpg.controller_crtft_type , acpg.controller_crtft_number)  =
             concat(ab4.id_type , ab4.id_num) 
         and ab4.status = '1' and ab4.back_flag = '0'
         where 1 = (select enable from aml_stcr where featurecode = '80023')
       and (select featureCodes from aml_susmodel where susModelCode = '1201') like '%80023%';
        ...
        inner join (select * from Aml_Blacklist where id_num is not null or id_num <> '') ab5
          on concat(acb.profit_crtft_type , acb.profit_crtft_no)  =
             concat(ab5.id_type , ab5.id_num) 
         and ab5.status = '1' and ab5.back_flag = '0'
        where 1 = (select enable from aml_stcr where featurecode = '80024')
       ...
       inner join (select * from Aml_Blacklist where id_num is not null or id_num <> '') ab6
          on concat(atr.other_customer_crtft_type ,atr.other_customer_crtft_number)  = concat(ab6.id_type , ab6.id_num) 
         and ab6.status = '1' and ab6.back_flag = '0'
        where 1 = (select enable from aml_stcr where featurecode = '80025')
       and (select featureCodes from aml_susmodel where susModelCode = '1201') like '%80025%';
        ...
        from aml_transaction_run atr
       inner join (select * from Aml_Blacklist where id_num is not null or id_num <> '') ab7
          on concat(atr.agent_user_crtft_type , atr.agent_user_crtft_number)  =
             concat(ab7.id_type , ab7.id_num) 
         and ab7.status = '1' and ab7.back_flag = '0'
        left join aml_stcr ams on ams.featureCode = '80026'
        where 1 = (select enable from aml_stcr where featurecode = '80026')
       ...
       where (ab1.customer_name is not null or
       ...
       inner join (select * from Aml_Blacklist where customer_name is not null or customer_name <> '') ab3
          on acpg.legal_name = ab3.customer_name
         and ab3.status = '1' and ab3.back_flag = '0'
       where 1 = (select enable from aml_stcr where featurecode = '80028')
       and (select featureCodes from aml_susmodel where susModelCode = '1201') like '%80028%';
        ...
        inner join (select * from Aml_Blacklist where customer_name is not null or customer_name <> '') ab4
			    on acpg.controller_name = ab4.customer_name
         and ab4.status = '1' and ab4.back_flag = '0'
        where 1 = (select enable from aml_stcr where featurecode = '80029')
       ...
       inner join (select * from Aml_Blacklist where customer_name is not null or customer_name <> '') ab5
          on acb.profit_name  = ab5.customer_name
         and ab5.status = '1' and ab5.back_flag = '0'
        where 1 = (select enable from aml_stcr where featurecode = '80030')
       ...
       inner join (select * from Aml_Blacklist where customer_name is not null or customer_name <> '') ab6
          on other_customer_name  = ab6.customer_name
         and ab6.status = '1' and ab6.back_flag = '0'
        where 1 = (select enable from aml_stcr where featurecode = '80031')
       and (select featureCodes from aml_susmodel where susModelCode = '1201') like '%80031%';
        ...
        inner join (select * from Aml_Blacklist where customer_name is not null or customer_name <> '') ab7
          on atr.agent_user_name = ab7.customer_name
         and ab7.status = '1' and ab7.back_flag = '0'
        where 1 = (select enable from aml_stcr where featurecode = '80032')
	...
	BEGIN
	  declare cur_suspcs_black_ cursor for
		SELECT t.trans_code,func_dist_str(GROUP_CONCAT(t.v_stcr),','),func_dist_str(GROUP_CONCAT(t.v_tosc),','),func_dist_str(GROUP_CONCAT(t.v_stcb),','),func_dist_str(GROUP_CONCAT(t.v_aosp),',') FROM aml_black_trans t GROUP BY t.trans_code;
	...
	create table aml_func_dist_str_tmp01(str1 varchar(2000));
		-- 汇总可疑特征码
	select func_dist_str(GROUP_CONCAT(distinct stcr),',') into v_stcr from aml_suspcs_case_detail where date_id = v_dt_date and suspcs_model_code = '1201';
	-- 汇总涉嫌犯罪类型
	select func_dist_str(GROUP_CONCAT(distinct tosc),',') into v_tosc from aml_suspcs_case_detail where date_id = v_dt_date and suspcs_model_code = '1201';
	-- 汇总资金交易行为情况
	select concat('交易总笔数',COUNT(1),'笔,交易金额',sum(trans_amount),'元') into v_stcb from aml_suspcs_case_detail where date_id = v_dt_date and suspcs_model_code = '1201';
	-- 汇总疑点分析
	select func_dist_str(GROUP_CONCAT(distinct aosp),',') into v_aosp from aml_suspcs_case_detail where date_id = v_dt_date and suspcs_model_code = '1201';	
  ...
  delete from aml_suspcs_case where date_id = v_dt_date and suspcs_model_code = '1201';
	...
	select count(1) into null_flag from aml_suspcs_case_detail where date_id = v_dt_date and suspcs_model_code = '1201';
