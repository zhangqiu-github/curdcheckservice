			...
			begin
				  ...
				  /* -------------------- 2001001|客户信息的公开程度-基本项完整度（对公） -------------------- */
				   ...
				   SELECT cols_name FROM aml_cust_risk_cols WHERE table_name = 'aml_customer_public' AND cols_isnull = 'is not null' order by cols_name;
			        ...
			        declare @num1 decimal(20,2);
			        declare @num2 decimal(20,2);
			        
			        select @num1 = count( 1 ) FROM aml_cust_risk_data WHERE cols_isnull > 0 
			        select @num2 = count( 1 ) FROM aml_cust_risk_data
				           ...
				           if cast(@subitem_weight as decimal(20,2)) = 0
							   ...
							   begin
									...
									if @num2 = 0
										...
										begin
				    						SELECT @v_risk_dg_2001001 = dbo.func_rule_compare_risk ( @num1 / @num2, '2001001' );
											-- 2001001|基本项完整度权重过滤
				   ...
				   /* -------------------- 2001002|客户信息的公开程度-是否上市(对公) -------------------- */
				    ...
				    select @subitem_weight =  SUM(cast(subitem_weight as decimal(20,2))) from aml_grade_weight where subitem_category = '001' and customer_type = '2'
				  ...
				  /* -------------------- 2006001|客户其他风险-非自然人客户的存续时间(对公) -------------------- */
			       ...
			       --获取大项总权重，如果权重为0，则不录入评分明细表
				    select @subitem_weight =  SUM(cast(subitem_weight as decimal(20,2))) from aml_grade_weight where subitem_category = '006' and customer_type = '2'
				  ...
				  /* -------------------- 2007001|反洗钱、反恐怖融资监控或制载情况-客户注册国籍(地区)受我国反洗钱监控或制载(对公) -------------------- */
			       ...
			       if @v_risk_dg_2007001 is null or @v_risk_dg_2007001 = ''
					...
					--获取大项总权重，如果权重为0，则不录入评分明细表
					select @subitem_weight =  SUM(cast(subitem_weight as decimal(20,2))) from aml_grade_weight where subitem_category = '007' and customer_type = '2'

					if cast(@subitem_weight as decimal(20,2)) > 0
						begin
						     
							-- 汇总 2007|反洗钱、反恐怖融资监控或制载情况（对公）
							set @v_risk_dg_2007 = cast(@v_risk_dg_2007001 as decimal(20,2));
						  
							-- 插入2007|反洗钱、反恐怖融资监控或制载情况(对公)分数
							insert into aml_grade_detail(auto_id,grade_code,customer_no,customer_name,grade_date,risk_section_code,risk_section_name,new_grade_score,system_grade_score,rule_source,grade_source,grade_cust,grade_reason)
							select newid() auto_id,
							@v_grade_code grade_code,
							customer_no customer_no,
				  ...
				  select @v_risk_dg_2008001 = case when subitem_score = '' then null else subitem_score end,@subitem_weight = cast(subitem_weight as decimal(20,2)) from aml_grade_weight where subitem_no = '2008001'
					...
					--获取大项总权重，如果权重为0，则不录入评分明细表
					select @subitem_weight =  SUM(cast(subitem_weight as decimal(20,2))) from aml_grade_weight where subitem_category = '008' and customer_type = '2'
				  ...
				  /******************************************* ********* *******************************************/
				   ...
				   select @v_risk_dg_2009001 = case when subitem_score = '' then null else subitem_score end,@subitem_weight = cast(subitem_weight as decimal(20,2)) from aml_grade_weight where subitem_no = '2009001'
					...
					--获取大项总权重，如果权重为0，则不录入评分明细表
					select @subitem_weight =  SUM(cast(subitem_weight as decimal(20,2))) from aml_grade_weight where subitem_category = '009' and customer_type = '2'
				  ...
				  /******************************************* ********* *******************************************/
				   ...
				   --获取大项总权重，如果权重为0，则不录入评分明细表
					select @subitem_weight =  SUM(cast(subitem_weight as decimal(20,2))) from aml_grade_weight where subitem_category = '016' and customer_type = '2'
				  ...
				  /******************************************* ********* *******************************************/
				   ...
				   if @v_risk_dg_2018001 is null or @v_risk_dg_2018001 = ''
				    ...
				    --获取大项总权重，如果权重为0，则不录入评分明细表
					select @subitem_weight =  SUM(cast(subitem_weight as decimal(20,2))) from aml_grade_weight where subitem_category = '018' and customer_type = '2'

					if cast(@subitem_weight as decimal(20,2)) > 0
						begin
						
						   -- 汇总 2018|是否属于公认的较高风险的行业 
						   set @v_risk_dg_2018 = cast(@v_risk_dg_2018001 as decimal(20,2));
						   -- 插入2018|是否属于公认的较高风险的行业(对公)分数
						   insert into aml_grade_detail(auto_id,grade_code,customer_no,customer_name,grade_date,risk_section_code,risk_section_name,new_grade_score,system_grade_score,rule_source,grade_source,grade_cust,grade_reason)
							select newid() auto_id,
							@v_grade_code grade_code,
							customer_no customer_no,
				  ...
				  if @v_risk_dg_2020001 is null or @v_risk_dg_2020001 = ''
				    ...
				    --获取大项总权重，如果权重为0，则不录入评分明细表
					select @subitem_weight =  SUM(cast(subitem_weight as decimal(20,2))) from aml_grade_weight where subitem_category = '020' and customer_type = '2'
				  ...
				  select @v_risk_dg_2999004 = case when subitem_score = '' then null else subitem_score end from aml_grade_weight where subitem_no = '2999004'
				   ...
				   -- 汇总  2999|附加项风险(对公) 
				   set @v_risk_dg_2999 = cast(@v_risk_dg_2999001 as decimal(20,2)) +  cast(@v_risk_dg_2999003 as decimal(20,2)) +  cast(@v_risk_dg_2999004 as decimal(20,2));
				  ...
				  -- 插入2999|附加项风险(对公)分数
				   insert into aml_grade_detail(auto_id,grade_code,customer_no,customer_name,grade_date,risk_section_code,risk_section_name,new_grade_score,system_grade_score,rule_source,grade_source,grade_cust,grade_reason)
					...
					'1' grade_source, -- 评分数据来源 系统评分
					'系统' grade_cust,
					'子项:2999001|附加项风险-客户姓名或证件存在于我国发布或承认的应实施反洗钱监控措施的名单(对公) 得分'+cast(@v_risk_dg_2999001 as varchar)+
					'；  子项:2999003|附加项风险-客户多次涉及可疑交易报告(对公) 得分'+cast(@v_risk_dg_2999003 as varchar)+
					'；  子项:2999004|附加项风险-客户配合金融机构依法开展的客户尽职调查工作(对公) 得分'+cast(@v_risk_dg_2999004 as varchar)+'；' grade_reason from Aml_Customer_Public_Run where date_id = @v_dt_date and customer_no = @v_customer_no;
