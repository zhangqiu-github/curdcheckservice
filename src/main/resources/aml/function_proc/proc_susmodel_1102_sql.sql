CREATE DEFINER=`aml`@`%` PROCEDURE `proc_susmodel_1102`(in dt_date varchar(20))
BEGIN
   /*
  可疑模型代码：1102
  ...
  */  
  ...
  insert into aml_transaction_run
    select *
     ...
     where suspcs_flag = '1'
       ...
       select atr.trans_code trans_code, -- 交易流水编号
        '80101' stcr, -- 可疑特征码
        ams.TOSCs tosc, -- 犯罪类型
        concat('交易金额',atr.trans_amount_cny,'元') stcb, -- 资金交易及客户行为情况
        '交易发生地涉嫌高危风险国家地区' aosp -- 疑点分析
        from aml_transaction_run atr
        left join aml_stcr ams on ams.featureCode = '80101'
       where concat(atr.trans_nationality,atr.trans_address) in (select concat(t.nationality_code,t.address_area) from Aml_High_Risk_Area t where address_status = '1') and 1 = (select enable from aml_stcr where featurecode = '80101')
    ...
    -- 对公客户注册地
      ...
      select atr.trans_code trans_code, -- 交易流水编号
       ...
       where concat(acpg.nationality_code1,acpg.register_area) in (select concat(t.nationality_code,t.address_area) from Aml_High_Risk_Area t where address_status = '1')
    ...
    -- 对公客户经营地
      ...
      select atr.trans_code trans_code, -- 交易流水编号
        ...
        ams.TOSCs tosc, -- 犯罪类型
        concat('交易金额',atr.trans_amount_cny,'元') stcb, -- 资金交易及客户行为情况
        '对公客户经营地涉嫌高危风险国家地区' aosp -- 疑点分析
        from aml_transaction_run atr
        left join aml_customer_public_run acpg
          on atr.customer_no = acpg.customer_no
         and acpg.date_id = v_dt_date
        left join aml_stcr ams on ams.featureCode = '80103'
       where concat(acpg.management_nationality_code1,acpg.management_area1) in (select concat(t.nationality_code,t.address_area) from Aml_High_Risk_Area t where address_status = '1')
    ...
    -- 对私客户出生地
       ...
       where concat(acps.nationality_code1,acps.register_area) in (select concat(t.nationality_code,t.address_area) from Aml_High_Risk_Area t where address_status = '1')
    ...
    -- 对私客户居住地
      insert into aml_black_trans
       select atr.trans_code trans_code, -- 交易流水编号
        ...
        ams.TOSCs tosc, -- 犯罪类型
        concat('交易金额',atr.trans_amount_cny,'元') stcb, -- 资金交易及客户行为情况
        '对私客户居住地涉嫌高危风险国家地区' aosp -- 疑点分析
        from aml_transaction_run atr
        left join aml_customer_personal_run acps
          on atr.customer_no = acps.customer_no
         and acps.date_id = v_dt_date
        left join aml_stcr ams on ams.featureCode = '80105'
       where concat(acps.management_nationality_code1,acps.management_area1) in (select concat(t.nationality_code,t.address_area) from Aml_High_Risk_Area t where address_status = '1')
    ...
    -- 交易对手所属地
      insert into aml_black_trans
       select atr.trans_code trans_code, -- 交易流水编号
        ...
        ams.TOSCs tosc, -- 犯罪类型
        concat('交易金额',atr.trans_amount_cny,'元') stcb, -- 资金交易及客户行为情况
        '交易对手所属地涉嫌高危风险国家地区' aosp -- 疑点分析
        from aml_transaction_run atr
        left join aml_stcr ams on ams.featureCode = '80106'
       where concat(atr.other_customer_nationality,atr.other_customer_area) in (select concat(t.nationality_code,t.address_area) from Aml_High_Risk_Area t where address_status = '1')
  ...
  select case when count(1) = 0 then 
   ...
   where date_id = v_dt_date and suspcs_model_code = '1102';
	...
	BEGIN
		declare cur_black_trans_ cursor for
    SELECT trans_code,func_dist_str(GROUP_CONCAT(t.v_stcr),','),func_dist_str(GROUP_CONCAT(t.v_tosc),','),func_dist_str(GROUP_CONCAT(t.v_stcb),','),func_dist_str(GROUP_CONCAT(t.v_aosp),',') FROM aml_black_trans t GROUP BY trans_code;
        
		declare continue handler for not FOUND set ret_cur_suspcs_black = 1;
		OPEN cur_black_trans_;
		-- 插入前删除事件交易明细记录
		delete FROM aml_suspcs_case_detail where date_id = v_dt_date and suspcs_model_code = '1102';
	  CUR_Loop:LOOP
	  FETCH cur_black_trans_ into v_trans_code,v_stcr,v_tosc,v_stcb,v_aosp;
     ...
     select uuid() AUTO_ID,t.* from (
        select distinct 
         ...
         left join aml_customer_public_run acpg
          on atr.customer_no = acpg.customer_no
         and acpg.date_id = v_dt_date
         where atr.trans_code = v_trans_code and atr.SUSPCS_FLAG = '1' 
         and atr.date_id = v_dt_date) t; 
	...
	create table aml_func_dist_str_tmp01(str1 varchar(2000));
		-- 汇总可疑特征码
	select func_dist_str(GROUP_CONCAT(distinct stcr),',') into v_stcr from aml_suspcs_case_detail where date_id = v_dt_date and suspcs_model_code = '1102';
	-- 汇总涉嫌犯罪类型
	select func_dist_str(GROUP_CONCAT(distinct tosc),',') into v_tosc from aml_suspcs_case_detail where date_id = v_dt_date and suspcs_model_code = '1102';
	-- 汇总资金交易行为情况
	select concat('交易总笔数',COUNT(1),'笔,交易金额' , sum(trans_amount), '元') into v_stcb from aml_suspcs_case_detail where  date_id = @v_dt_date and suspcs_model_code = '1102';
	-- 汇总疑点分析
	select func_dist_str(GROUP_CONCAT(distinct aosp),',') into v_aosp from aml_suspcs_case_detail where date_id = v_dt_date and suspcs_model_code = '1102';	
					
		-- 输出可疑事件表
  delete from aml_suspcs_case where date_id = v_dt_date and suspcs_model_code = '1102';
	select count(1) into null_flag from aml_suspcs_case_detail where date_id = v_dt_date and suspcs_model_code = '1102';
	if null_flag > 0 then
