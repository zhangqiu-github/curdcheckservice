   ...
   where date_id = v_dt_date and suspcs_model_code = '1105';
		...
		BEGIN
		 ...
		 declare cur_sus_trans_ cursor for
			 ...
			 -- 已审核
	  	 select ag1.customer_no,'80401' stcr,ams.TOSCs tosc,'风险评级为高风险的客户交易' stcb,'客户评级审核状态包括未审核、初审通过、复审驳回、复审通过、终审驳回且状态为有效的高风险客户交易' aosp 
			 from aml_grade ag1
       left join aml_stcr ams on ams.featureCode = '80401'
       ...
       and (select featureCodes from aml_susmodel where susModelCode = '1105') like '%80401%';
     ...
     select uuid() AUTO_ID,t.* from (
        select distinct 
               ...
               v_tosc TOSC, -- 可疑犯罪类型
               v_stcr STCR, -- 可疑特征码
               concat('交易金额',atr.trans_amount_cny,'元') STCB, -- 客户资金行为情况
               v_aosp AOSP, -- 疑点分析
	...
	-- 汇总资金交易行为情况
	select concat('交易总笔数',COUNT(1),'笔,交易金额',sum(trans_amount),'元') into v_stcb from aml_suspcs_case_detail where date_id = v_dt_date and suspcs_model_code = '1105';
	-- 汇总疑点分析
